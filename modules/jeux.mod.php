	...
	function corps()
	{
		if(isset($_GET['id']))
		{
			...
			if(!empty($record['id_user']) && $jeu['concours']==1) 
			{
				...
				$phrase="<p>Record : ".$record['score']." d�tenu par ".$recordman['login']."</p>";
			}
...
<?php	  
		...
		{
			echo "\n<p>Si tu as un peu de temps � tuer ou si le yetisport n'a plus de secret pour toi tu tombes � pic ! 
					Entra�nes toi dans la section <a href=\"?menu=bonus&module=jeux&action=fun\">Just for fun</a> et quand 
					tu seras pr�t(e) viens d�fier les meilleur(e)s dans notre
